...
def dispatch_tensorflowjs_to_keras_saved_model_conversion(
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session():
        model = keras_tfjs_loader.load_keras_model(config_json_path)
        tf.keras.models.save_model(model, keras_saved_model_path, save_format="tf")
...
def _standardize_input_output_formats(input_format, output_format):
    ...
    """
    input_format_is_keras = input_format in [
        common.KERAS_MODEL,
        common.KERAS_SAVED_MODEL,
    ]
    input_format_is_tf = input_format in [
        common.TF_SAVED_MODEL,
        common.TF_FROZEN_MODEL,
        common.TF_HUB_MODEL,
    ]
...
def _dispatch_converter(
    ...
    #   branches below.
    if input_format == common.KERAS_MODEL and output_format == common.TFJS_LAYERS_MODEL:
        dispatch_keras_h5_to_tfjs_layers_model_conversion(
            args.input_path,
        ...
        input_format == common.KERAS_MODEL and output_format == common.TFJS_GRAPH_MODEL
    ):
        dispatch_keras_h5_to_tfjs_graph_model_conversion(
            args.input_path,
            output_dir=args.output_path,
        ...
        input_format == common.TF_SAVED_MODEL
        ...
        input_format == common.TF_HUB_MODEL and output_format == common.TFJS_GRAPH_MODEL
    ):
        ...
        input_format == common.TFJS_LAYERS_MODEL and output_format == common.KERAS_MODEL
    ):
        dispatch_tensorflowjs_to_keras_h5_conversion(args.input_path, args.output_path)
    elif (
        input_format == common.TFJS_LAYERS_MODEL
        ...
        input_format == common.TF_FROZEN_MODEL
        and output_format == common.TFJS_GRAPH_MODEL
    ):
...
def get_arg_parser():
    ...
    parser.add_argument(
        common.OUTPUT_PATH, nargs="?", type=str, help="Path for all output artifacts."
    )
    parser.add_argument(
...
def convert(arguments):
    ...
    if not args.input_path:
        raise ValueError("Missing input_path argument. For usage, use the --help flag.")
    ...
    if args.weight_shard_size_bytes is not None:
        if output_format not in (common.TFJS_LAYERS_MODEL, common.TFJS_GRAPH_MODEL):
            raise ValueError(
        ...
        if not (
            isinstance(args.weight_shard_size_bytes, int)
